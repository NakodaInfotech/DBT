...
{
    public class SaleGatePassUpdateController : ApiController
    {
        DataSet ds;
        ...
        [HttpPost]
        public HttpResponseMessage SaleGatePassUpdate([FromBody] LoginModel UserData)
        {
            try
            {
                HttpResponseMessage response = Request.CreateResponse(HttpStatusCode.NotFound, "ERROR");
                UserData.SO_Status = "ERROR";
                ds = objDL.GetSaleGatePassUpdate(UserData);
