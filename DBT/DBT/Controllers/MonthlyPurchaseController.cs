...
{
    public class MonthlyPurchaseController : ApiController
    {
        DataSet ds;
        ...
        [HttpPost]
        public HttpResponseMessage GetMonthlyPurchase([FromBody] LoginModel UserData)
        {
            try
            {
                HttpResponseMessage response = Request.CreateResponse(HttpStatusCode.NotFound, "ERROR");
                UserData.SO_Status = "ERROR";
                ds = objDL.GetMonthlyPurchase(UserData);
