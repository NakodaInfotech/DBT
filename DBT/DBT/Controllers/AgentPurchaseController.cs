...
{
    public class AgentPurchaseController : ApiController
    {
        DataSet ds;
        ...
        [HttpPost]
        public HttpResponseMessage GetAgentPurchase([FromBody] LoginModel UserData)
        {
            try
            {
                HttpResponseMessage response = Request.CreateResponse(HttpStatusCode.NotFound, "ERROR");
                UserData.SO_Status = "ERROR";
                ds = objDL.GetAgentPurchase(UserData);
