...
{
    public class BarcodeDetailsController : ApiController
    {
        DataSet ds;
        ...
        [HttpPost]
        public HttpResponseMessage GetBarcodeDetails([FromBody] LoginModel UserData)
        {
            try
            {
                HttpResponseMessage response = Request.CreateResponse(HttpStatusCode.NotFound, "ERROR");
                UserData.SO_Status = "ERROR";
                ds = objDL.GetBarcodeDetails(UserData);
