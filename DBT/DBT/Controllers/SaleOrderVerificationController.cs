...
{
    public class SaleOrderVerificationController : ApiController
    {
        DataSet ds;
        ...
        [HttpPost]
        public HttpResponseMessage OrderVerification([FromBody] LoginModel UserData)
        {
            try
            {
                HttpResponseMessage response = Request.CreateResponse(HttpStatusCode.NotFound, "ERROR");
                UserData.SO_Status = "ERROR";
                ds = objDL.OrderVerification(UserData);
