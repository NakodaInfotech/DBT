...
{
    public class AllPurchaseBillsController : ApiController
    {
        DataSet ds;
        ...
        [HttpPost]
        public HttpResponseMessage GetAllPurchaseBills([FromBody] LoginModel UserData)
        {
            try
            {
                HttpResponseMessage response = Request.CreateResponse(HttpStatusCode.NotFound, "ERROR");
                UserData.SO_Status = "ERROR";
                ds = objDL.GetAllPurchaseBills(UserData);
