...
{
    public class TopItemPurchaseController : ApiController
    {
        DataSet ds;
        ...
        [HttpPost]
        public HttpResponseMessage GetTopItemPurchase([FromBody] LoginModel UserData)
        {
            try
            {
                HttpResponseMessage response = Request.CreateResponse(HttpStatusCode.NotFound, "ERROR");
                UserData.SO_Status = "ERROR";
                ds = objDL.GetTopItemPurchase(UserData);
