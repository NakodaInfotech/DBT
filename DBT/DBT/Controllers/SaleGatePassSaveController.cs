...
{
    public class SaleGatePassSaveController : ApiController
    {
        DataSet ds;
        ...
        [HttpPost]
        public HttpResponseMessage SaleGatePassSave([FromBody] LoginModel UserData)
        {
            try
            {
                HttpResponseMessage response = Request.CreateResponse(HttpStatusCode.NotFound, "ERROR");
                UserData.SO_Status = "ERROR";
                ds = objDL.GetSaleGatePassSave(UserData);
